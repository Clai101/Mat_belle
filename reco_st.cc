...
{ extern BelleTupleManager* BASF_Histogram;    
  t1 = BASF_Histogram->ntuple ("lmbda_lept",
    "ml mach p chu chl chach en ecm ntr rm2n fox rm2l rm2nu chrgl chrgach en_gam enc_gam");
  /*
...
void User_reco::event ( BelleEvent* evptr, int* status ) {
  ...
  *status=0;
  ...
  std::vector<Particle> rho4, rho_ppm, rho_mmp, rho_pp, rho_mm, rho;
  ...
  combination(D0, m_ptypeD0, k_s, pi_p, pi_m, pi0, 0.05);
  combination(aD0, m_ptypeD0B, k_s, pi_p, pi_m, pi0, 0.05);
  ...
  setUserInfo(D_p_st, {{"chanel", 1}, {"charg", 0}, {"barion_num", 0}});
  setUserInfo(D_m_st, {{"chanel", 1}, {"charg", 0}, {"barion_num", 0}});

  combination(D_p_st, m_ptypeDstar0, D_p, pi0, 0.05);
  combination(D_m_st, m_ptypeDstarB, D_m, pi0, 0.05);
  setUserInfo(D_p_st, {{"chanel", 2}, {"charg", 0}, {"barion_num", 0}});
  setUserInfo(D_m_st, {{"chanel", 2}, {"charg", 0}, {"barion_num", 0}});

  for(std::vector<Particle>::iterator l = D0_st.begin(); l!=D0_st.end(); ++l) {
    D0_st.
    if (aD0_st.child(0)) {
      D0_st.erase(l); --l; continue;
    }
  }
  ...
  combination(ups, m_ptypeUPS4, lamc_p, ap, D_m_st, pi_p);
  combination(ups, m_ptypeUPS4, lamc_m, p, D_p_st, pi_m);
  setUserInfo(ups, {{"chanel", 1}});
  
  combination(ups, m_ptypeUPS4, lamc_p, ap, aD0_st);
  combination(ups, m_ptypeUPS4, lamc_m, p, D0_st);
  setUserInfo(ups, {{"chanel", 2}});

  combination(ups, m_ptypeUPS4, lamc_p, ap, D_m, pi_p);
  combination(ups, m_ptypeUPS4, lamc_m, p, D_p, pi_m);
  setUserInfo(ups, {{"chanel", 3}});
  
  combination(ups, m_ptypeUPS4, lamc_p, ap, aD0);
  combination(ups, m_ptypeUPS4, lamc_m, p, D0);
